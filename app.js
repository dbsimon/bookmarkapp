...
function renderCategoryTabs(){
  ...
  const tabs = [
    ...
    ...state.categories.slice().sort((a,b)=>a.sortOrder-b.sortOrder).map(c => ({
      id:c.id, label:`${c.icon || '📁'} ${c.name} (${counts.byCategory[c.id] || 0})`
    })),
...
function openTool(id){
  ...
  saveState();
  renderAll();
  if(tool.openMode === 'same_tab') window.location.href = tool.url;
...
function slugify(s){
  return s.toLowerCase().trim().replace(/[^a-z0-9\u4e00-\u9fa5]+/g,'-').replace(/^-+|-+$/g,'') || uid('cat');
}
